# Scrap and Interpolate PM2.5 Data from udara.jakarta.go.id
...
2. **Spatial Analysis**: An interpolation system that transforms point-based sensor measurements into pollution estimates for all Kelurahan in Jakarta (this version only sucesfully recovers 260 Kelurahan)
...
2. **Sensor Location Validation**
   ...
   - Valid latitude range: -7° to -5.4° S (extended to include Kep. Seribu )
   - Exclude sensors with coordinates outside these bounds

3. **Reporting Frequency Analysis** (need to edit based on the latest imputation logic)
...
4. **Timestamp Standardization** (need to edit based on the latest imputation logic)
...
5. **Temporal Interpolation** (need to edit based on the latest imputation logic)
...
7. **Spatial Grid Creation**
   - Create a grid of points covering all Jakarta kelurahan with a cell size of 0.005 degrees (roughly XX km)
   - This grid forms the basis for interpolation calculations

8. **IDW Interpolation**
   - For each grid point, use the 10 nearest sensors to estimate PM2.5 values (need to edit based on the latest imputation logic)
...
9. **Kelurahan-level Aggregation**
   ...
   - Calculate average PM2.5 value for each kelurahan based on contained grid points (is this simple average across grids or weight grids based its overlap with Kelurahan boundaries?)
